...
const ItineraryForm = (props) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (props.match.params.itemId) {
      ItineraryManager.updateItineraryItem(
        props.match.params.itemId,
        itineraryItemObj
      ).then(() => props.history.push("/myitinerary"));
    } else {
      ItineraryManager.createItineraryItem(itineraryItemObj).then(() =>
  ...
  useEffect(() => {
    if (props.match.params.itemId) {
      ItineraryManager.getItineraryItem(props.match.params.itemId).then(
        (item) => {
          const stateToChange = { ...itineraryItem };
    ...
    ParkManager.getAttractions().then((allAttractions) => {
